...
data_dir = '/home/srgrace/genericContest_data/HE_flower_recognition/'
...
def pre_process(df, folder):
    ...
    for i in tqdm(range(df.shape[0])):
        ...
        img = image.load_img(os.path.join(data_dir, folder) + df['image_id'][i].astype('str') + '.jpg',
...
def training():
    ...
    y = to_categorical(y)
    # print(y, train['category'].shape, len(y))
    # exit()
    ...
    model.save(os.path.join('./data/', 'seq.h5'))
...
def predict():
    ...
    model = load_model('./data/seq.h5')
    prediction = model.predict_classes(x)

    test['category'] = prediction
    test.to_csv('./data/predict.csv', header=True, index=False)
